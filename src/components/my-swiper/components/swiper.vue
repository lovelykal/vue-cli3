<template>
    <section class="wh_content" @touchmove="fn">
        ...
        <div v-if="showIndicator" class="wh_indicator">
            <div v-for="(tag,$index) in slidesLength" :key="tag" v-bind:class="{ wh_show_bgcolor: index-1==$index }" class="wh_indicator_item"></div>
        </div>
    </section>
</template>
...
export default {
  ...
  mounted () {
    ...
    setTimeout(() => {
      // 克隆dom
      this.starDom()
      this.dom.transform = `translate3d(${this.width1 * -1}px, 0px, 0px)`
      this.dom['-webkit-transform'] = `translate3d(${this.width1 * -1}px, 0px, 0px)`
      this.dom['-ms-transform'] = `translate3d(${this.width1 * -1}px, 0px, 0px)`
      if (this.autoPlay) {
        this.setTime()
      }
    }, 50)
  ...
  methods: {
    ...
    m (x) {
      ...
      if (this.slideing && this.t.s !== -1) {
        this.clearTimeOut()
        this.t.m = x.touches[x.touches.length - 1].cilentX - this.t.s
    ...
    e (x) {
      if (this.slideing && this.t.s !== -1) {
        ...
        a += this.t.m > 0 ? this.width1 * 0.3 : this.width1 * -0.3
        this.index = Math.round(a / this.width1) * -1
        this.wh('touch')
      }
    },
    setTransform (num) {
    ...
    wh (type) {
      ...
      this.setTransform(this.index * -1 * this.width1)
      this.t.m = 0
      this.t.s = -1 // 保证下次重新赋值
      if (this.autoPlay) {
        this.setTime()
    ...
    starDom () {
      ...
      if (this.slidesLength > 1) {
        ...
        this.width1 = document.querySelector('.' + this.className).offsetWidth
